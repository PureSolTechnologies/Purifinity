...
public class SourceCodeAnalysisDistributionIT {
	...
	public void testJavaOnSourceCodeAnalysisCode() throws IOException {
		...
		FileSearchConfiguration configuration = new FileSearchConfiguration(
				...
				excludeFiles, true);
		FileTree fileTree = FileTreeSearch
				.getFileTree(directory, configuration);
	...
	private static boolean parseFile(File file) throws IOException {
		...
		SourceFileLocation sourceFileLocation = new SourceFileLocation(dir,
				fileName);
		try (InputStream sourceStream = sourceFileLocation.openStream()) {
			CodeAnalyzer analyser = java.createAnalyser(
					SourceCode.read(sourceStream, sourceFileLocation), null);
			analyser.analyze();
			return analyser.getAnalysis().getAnalysisInformation()
					.isSuccessful();
		}
	}
}

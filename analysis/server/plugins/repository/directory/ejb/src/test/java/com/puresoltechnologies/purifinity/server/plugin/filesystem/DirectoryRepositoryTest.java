...
public class DirectoryRepositoryTest {
	...
	parameters.put("parameter1", new ParameterWithArbitraryUnit<Double>("parameterName", "parameterUnit",
		LevelOfMeasurement.NOMINAL, "parameterDescription", Double.class));
	RepositoryServiceInformation repositoryType = new RepositoryServiceInformation("className", "name", "version",
		new Version(1, 2, 3), "jndi://", "description", parameters, new ConfigurationParameter<?>[] {}, null,
		null, null);
	...
	RepositoryServiceInformation repositoryType = DirectoryRepository.INFORMATION;
	checkSerialization(repositoryType);
    }
	...
	RepositoryServiceInformation deserialized = JSONSerializer.fromJSONString(serialized,
		RepositoryServiceInformation.class);
	assertNotNull(deserialized);
	assertEquals(repositoryType.getId(), deserialized.getId());
	assertEquals(repositoryType.getName(), deserialized.getName());
	assertEquals(repositoryType.getDescription(), deserialized.getDescription());
	assertEquals(repositoryType.getParameters(), deserialized.getParameters());
    }
}

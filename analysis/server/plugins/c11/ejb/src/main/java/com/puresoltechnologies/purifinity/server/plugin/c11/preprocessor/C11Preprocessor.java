...
public class C11Preprocessor implements Preprocessor {
     ...
     */
    public C11Preprocessor(IncludeDirectories includeDirectories, DefinedMacros definedMacros) {
     ...
     */
    public C11Preprocessor(IncludeDirectories includeDirectories, DefinedMacros definedMacros, int nestingDepth) {
    ...
    /**
     * 
     * This method overrides {@link C11Preprocessor#process(SourceCode)}.
     * 
     * Here we have three things to do:
     * <ol>
     * <li>We assure the presence of a line terminator in the last line of the
     * source code.</li>
     * <li>The processing of the source code is started.</li>
     * <li>If a line terminator was added before, we remove the line terminator
     * again to reset to the original code.</li>
     * </ol>
     * 
     * @param sourceCode
     *            is the {@link SourceCode} to process.
     * @return The {@link SourceCode} which was processed is returned.
     */
    @Override
    public SourceCode process(SourceCode sourceCode) throws PreprocessorException {
     ...
     * 
     * @param sourceCode
     *            is the {@link SourceCode}.
     * @return <code>true</code> is returned in case a line terminator was
     *         added.
     */
	...
	lastLine = new SourceCodeLine(lastLine.getSource(), lastLine.getLineNumber(), text + "\n");
	lines.add(lastLine);
	/*
     ...
     */
    private SourceCode performPreprocessing(SourceCode sourceCode) throws PreprocessorException {
	try {
	    C11PreprocessorParser parser = new C11PreprocessorParser();
	    ParseTreeNode ast = parser.parse(sourceCode);
	    TreeMacroProcessor processor = new TreeMacroProcessor(ast, includeDirectories, definedMacros, nestingDepth);
	    processor.process();
	    return processor.getSourceCode();
	} catch (ParserException e) {
	    throw new PreprocessorException("Could not preprocess source code!", e);

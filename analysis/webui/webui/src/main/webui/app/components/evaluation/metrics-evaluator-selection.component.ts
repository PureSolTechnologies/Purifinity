 ...
 */
@Component({
    selector: 'metrics-evaluator-selection',
    template:
    `<select 
...
export class MetricsEvaluatorSelectionComponent {
    ...
    constructor(private pluginManager: PluginManager) {
        ...
        pluginManager.getEvaluators(
            function(evaluators: EvaluatorServiceInformation[]) {
                evaluators.forEach(function(evaluator: EvaluatorServiceInformation) {
                    if (evaluator.type === "METRICS") {
                        component.evaluators.push(evaluator);
                    }
                });

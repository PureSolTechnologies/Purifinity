...
public abstract class AbstractTokenDefinition implements TokenDefinition {
	...
	protected AbstractTokenDefinition() {
		initialize();
		compilePattern();
	}
	...
	private void compilePattern() {
		if (isCaseSensitive) {
			...
			startPattern = Pattern.compile("^(" + patternString
					+ lookAheadPatternString + ")");
		} else {
			pattern = Pattern.compile("(" + patternString
					+ lookAheadPatternString + ")", Pattern.CASE_INSENSITIVE);
			startPattern = Pattern.compile("^(" + patternString
					+ lookAheadPatternString + ")", Pattern.CASE_INSENSITIVE);
		}
	}
	...
	public final int hashCode() {
		...
		result = prime * result
				+ ((startPattern == null) ? 0 : startPattern.hashCode());
		return result;
	}

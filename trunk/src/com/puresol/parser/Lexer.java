...
public class Lexer {
	...
	public void addDefinitions(Class<? extends TokenDefinitions> definitions) {
		try {
			Constructor<?> constructor = definitions.getConstructor();
			TokenDefinitions definitionsInstance = (TokenDefinitions) constructor
					.newInstance();
			...
			while (pos < text.length()) {
				...
				for (TokenDefinition definition : tokenDefinitions) {
					if (definition.atStart(text.substring(pos))) {
						...
						outputStream.addToken(new Token(token.getStream(), id,
								definition.getPublicity(), pos, tokenText
										.length(), tokenText, lineNumber,

...
public class Lexer {
	...
	private ArrayList<TokenDefinition> tokenDefinitions = new ArrayList<TokenDefinition>();
	...
	public void addDefinitions(Class<? extends TokenDefinitionGroup> definitions) {
		try {
		...
		while (pos < text.length()) {
			...
			for (TokenDefinition definition : tokenDefinitions) {
				if (definition.atStart(text.substring(pos))) {
					...
					outputStream.addToken(new Token(id, definition
							.getPublicity(), pos, tokenText.length(),
							tokenText, lineNumber, lineNumber + numberOfLines,
							definition.getClass()));
					id++;
			...
			if (!found) {
				throw new NoMatchingTokenDefinitionFound(lineNumber, pos, text);
			}
		}
		return lineNumber;
	}

 ...
 * of code. It's a simple counting statistics for productive, comment and
 * blank lines.
 * 
...
public class SLOCMetric extends AbstractMetric {
	...
	while (!previous.getText().equals("\n")) {
	    if (previous.getChannel() == Token.HIDDEN_CHANNEL) {
	...
	for (String line : lines) {
	    ...
	    lineLengthsTrimmedProductive.add(trimmed
		    .length());
	}
	lineStatistics =
		new Statistics(toDouble(lineLengths));
	trimmedLineStatistics =
		new Statistics(toDouble(lineLengthsTrimmed));
	productiveLineStatistics =
		new Statistics(
			toDouble(lineLengthsProductive));
	trimmedProductiveLineStatistics =
		new Statistics(
	...
	QualityLevel levelLineCount =
		getQualityLevelLineCount();
	QualityLevel levelLineLength =
		this.getQualityLevelLineLength();
	if (levelLineCount.getLevel() < levelLineLength
		.getLevel()) {
	...
	return HTMLStandards
		.convertTSVToTable(getLineCountReport());
    }
	...
	Statistics prodTrimmed =
		getTrimmedProductiveLineStatistics();
	String report =
		"\tnormal\ttrimmed\tproductive"
			+ "\tproductive trimmed\n";
	report +=
		"avg\t"
			+ Math
				.round(normal.getAvg() * 100.0)
			/ 100.0
			+ "\t"
			+ Math
				.round(trimmed.getAvg() * 100.0)
			/ 100.0
			+ "\t"
			+ Math.round(prod.getAvg() * 100.0)
			/ 100.0
			+ "\t"
			+ Math
				.round(prodTrimmed.getAvg() * 100.0)
			/ 100.0 + "\n";
	report +=
		"median\t"
			+ Math.round(normal.getMedian())
			+ "\t"
			+ Math.round(trimmed.getMedian())
			+ "\t"
			+ Math.round(prod.getMedian())
			+ "\t"
			+ Math.round(prodTrimmed
				.getMedian()) + "\n";
	report +=
		"standard deviation\t"
			+ Math
				.round(normal.getStdDev() * 100.0)
			/ 100.0
			+ "\t"
			+ Math
				.round(trimmed.getStdDev() * 100.0)
			/ 100.0
			+ "\t"
			+ Math
				.round(prod.getStdDev() * 100.0)
			/ 100.0
			+ "\t"
			+ Math.round(prodTrimmed
				.getStdDev() * 100.0)
	...
	return HTMLStandards
		.convertTSVToTable(getLineLengthReport());
    }
}

...
public class HalsteadMetric extends AbstractMetric {
    ...
    private Hashtable<String, Integer> operands =
	    new Hashtable<String, Integer>();
	...
	if (operands.containsKey(operand)) {
	    operands
		    .put(operand, operands.get(operand) + 1);
	} else {
	...
	N = N1 + N2;
	HL =
		n1 * Math.log(n1) / Math.log(2) + n2
			* Math.log(n2) / Math.log(2);
	...
	String report =
		"n1\t"
			...
			+ "\t"
			+ translator
				.i18n("Number of different operators")
			+ "\n";
	report +=
		"N1\t"
			+ get_N1()
	...
	report +=
		"n2\t"
			...
			+ "\t"
			+ translator
				.i18n("Number of different operands")
			+ "\n";
	report +=
		"N2\t"
			+ get_N2()
			+ "\t"
			+ translator
				.i18n("Total number of operands")
			+ "\n";
	report +=
		"n\t"
			+ Math.round(get_n() * 100.0)
			/ 100.0
			+ "\t"
			+ translator
	...
	report +=
		"HL\t"
			+ Math.round(get_HL() * 100.0)
			/ 100.0
			+ "\t"
			+ translator
				.i18n("Halstead length")
			+ "\n";
	report +=
		"HV\t"
			+ Math.round(get_HV() * 100.0)
			/ 100.0
			+ "\t"
			+ translator
				.i18n("Halstead volume")
			+ "\n";
	report +=
		"D\t"
			+ Math.round(get_D() * 100.0)
			/ 100.0
			+ "\t"
			+ translator
	...
	report +=
		"E\t"
			+ Math.round(get_E() * 100.0)
			/ 100.0
			+ "\t"
			+ translator
				.i18n("Effort to implement")
			+ "\n";
	report +=
		"T\t"
			+ Math.round(get_T() * 100.0)
			/ 100.0
			+ "\t"
			+ translator
				.i18n("Implementatiom time [s]")
			+ "\n";
	report +=
		"B\t"
			+ Math.round(get_B() * 100.0)
			/ 100.0
			+ "\t"
			+ translator
				.i18n("Number of delivered bugs")
			+ "\n";
	return report;
    }

...
class_def
  :
  annotation* class_modifier* class_ id generic? (extends_ class_name)? (implements_ class_name (comma class_name)*)? class_block {helper.registerRange(CodeRangeType.CLASS, $id.text, $text, retval.start.getTokenIndex(), input.LT(-1).getTokenIndex());}
  ;

enum_def
  :
  annotation* class_modifier* enum_ id generic? (extends_ class_name)? enum_block {helper.registerRange(CodeRangeType.ENUMERATION, $id.text, $text, retval.start.getTokenIndex(), input.LT(-1).getTokenIndex());}
  ;

interface_def
  :
  annotation* class_modifier* interface_ id generic? (extends_ class_name)? interface_block {helper.registerRange(CodeRangeType.INTERFACE, $id.text, $text, retval.start.getTokenIndex(), input.LT(-1).getTokenIndex());}
  ;

annotation_def
  :
  annotation* class_modifier* annotation_ id interface_block {helper.registerRange(CodeRangeType.INTERFACE, $id.text, $text, retval.start.getTokenIndex(), input.LT(-1).getTokenIndex());}
  ;
...
constructor_def
  :
  annotation* method_modifier* id open_bracket argument_def close_bracket throws_exception? code {helper.registerRange(CodeRangeType.CONSTRUCTOR, $id.text, $text, retval.start.getTokenIndex(), input.LT(-1).getTokenIndex());}
  ;

method_def
  :
  annotation* method_modifier* variable_type id open_bracket argument_def close_bracket throws_exception? code {helper.registerRange(CodeRangeType.METHOD, $id.text, $text, retval.start.getTokenIndex(), input.LT(-1).getTokenIndex());}
  ;
...
method_call
  :
  method_name open_bracket arguments close_bracket
  (
    dot
    (
      id
...
value
  ...
  right_unary? (binary_operator value)? (question_ value colon value)?
  | left_unary?
  (
    single_value
    | open_bracket value close_bracket
...
for_loop
  :
  for_ OPEN_BRACKET (variable_def (comma variable_def)*)? semicolon value? semicolon (statement_wosemicolon (comma statement_wosemicolon)*)? CLOSE_BRACKET code
...
cast
  :
  OPEN_BRACKET variable_type CLOSE_BRACKET {helper.registerOperator("(cast)");}
  ;
...
id
  :
  ID {helper.registerOperant($text);}
  ;
...
class_
  :
  CLASS {helper.registerOperator($text);}
  ;

enum_
  :
  ENUM {helper.registerOperator($text);}
  ;
...
extends_
  :
  EXTENDS {helper.registerOperator($text);}
  ;
...
this_
  :
  THIS {helper.registerOperant($text);}
  ;

super_
  :
  SUPER {helper.registerOperant($text);}
  ;

void_
  :
  VOID {helper.registerOperant($text);}
  ;

public_
  :
  PUBLIC {helper.registerOperator($text);}
  ;

private_
  :
  PRIVATE {helper.registerOperator($text);}
  ;
...
static_
  :
  STATIC {helper.registerOperator($text);}
  ;

final_
  :
  FINAL {helper.registerOperator($text);}
  ;
...
new_
  :
  NEW {helper.registerOperator($text);}
  ;

try_
  :
  TRY {helper.registerOperator($text);}
  ;

catch_
  :
  CATCH {helper.registerOperator($text, 1);}
  ;

finally_
  :
  FINALLY {helper.registerOperator($text, 1);}
  ;

throws_
  :
  THROWS {helper.registerOperator($text);}
  ;

throw_
  :
  THROW {helper.registerOperator($text);}
  ;

for_
  :
  FOR {helper.registerOperator("for()", 1);}
  ;

while_
  :
  WHILE {helper.registerOperator("while()", 1);}
  ;

do_
  :
  DO {helper.registerOperator("do()", 1);}
  ;

if_
  :
  IF {helper.registerOperator("if()", 1);}
  ;

else_
  :
  ELSE {helper.registerOperator("else");}
  ;

switch_
  :
  SWITCH {helper.registerOperator("switch()");}
  ;

case_
  :
  CASE {helper.registerOperator("case", 1);}
  ;

default_
  :
  DEFAULT {helper.registerOperator("default", 1);}
  ;

return_
  :
  RETURN {helper.registerOperator($text);}
  ;

break_
  :
  BREAK id {helper.registerOperator($BREAK.text);}
...
comma
  :
  COMMA {helper.registerOperator($text);}
  ;

colon
  :
  COLON {helper.registerOperator($text);}
  ;
...
dot
  :
  DOT {helper.registerOperator($text);}
  ;

...
public class DuplicationScanner implements ProgressObservable {
	...
	if (counter > OPERATOR_THRESHOLD) {
	    CodeRange leftDuplication =
		    new CodeRange(leftStream.getFile(), left.getType(),
			    left.getName(), leftStream, leftIndex,
			    leftToken.getTokenID());
	    CodeRange rightDuplication =
		    new CodeRange(rightStream.getFile(), right.getType(),
			    right.getName(), rightStream, rightIndex,

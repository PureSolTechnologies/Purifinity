...
public class HTMLAnalysisReport extends AbstractAnalysisReport {
	...
	private String getOverview() {
		...
		AbstractMaintainabilityIndex maintainability = getMetrics()
				.getMaintainabilityIndex();
		AbstractEntropyMetric entropy = getMetrics().getEntropyMetric();
		report += "<table>";
		if (sloc != null) {
			report += "<tr><td>SLOC Metric</td><td>"
					+ ReportStandards.getQualitySign(sloc) + "</td><td>"
					+ translator.i18n("Statistics on source code fo lines")
					+ "</td></tr>";
		}
		if (mcCabe != null) {
			report += "<tr><td>McCabe Metric</td><td>"
					+ ReportStandards.getQualitySign(mcCabe) + "</td><td>"
					+ translator.i18n("Cyclomatic number") + "</td></tr>";
		}
		if (halstead != null) {
			report += "<tr><td>Halstead Metric</td><td>"
					+ ReportStandards.getQualitySign(halstead) + "</td><td>"
					+ translator.i18n("Statistics on operators and operands")
					+ "</td></tr>";
		}
		if (maintainability != null) {
			report += "<tr><td>Maintainability Index</td><td>"
					+ ReportStandards.getQualitySign(maintainability)
					+ "</td><td>" + translator.i18n("Maintainability Index")
					+ "</td></tr>";
		}
		if (entropy != null) {
	...
	private String getMetricReport() {
		String report = SLOCReport.getHTMLReport(getMetrics().getSLOCMetric());
		report += McCabeReport.getHTMLReport(getMetrics().getMcCabeMetric());
		report += HalsteadReport
				.getHTMLReport(getMetrics().getHalsteadMetric());
		report += MaintainabilityReport.getHTMLReport(getMetrics()
				.getMaintainabilityIndex());
		report += EntropyReport.getHTMLReport(getMetrics().getEntropyMetric());

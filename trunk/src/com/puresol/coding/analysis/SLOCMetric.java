...
public class SLOCMetric extends AbstractMetric {
    ...
    /**
     * This method calculates how much lines are encoded in the current
     * token and the tokens before which do not end with a new line.
     * 
     * @param token
     * @return
     */
	...
	/*
	 * A blank lines consists only of channel 99 tokens with are
	 * trimmed empty.
	 */
	int index = token.getTokenID();
	if (index == 0) {
	...
	/*
	 * A productive line is a line with at least one token not in
	 * channel 99!
	 */
	int index = token.getTokenID();
	if (index == 0) {
	...
	productiveLineStatistics =
		new Statistics(toDouble(lineLengthsProductive));
	trimmedProductiveLineStatistics =
		new Statistics(toDouble(lineLengthsTrimmedProductive));
    }
     ...
     * 
     * @see
     * com.puresol.coding.analysis.SLOCMetric#getTrimmedLineStatistics()
     */
     ...
     * 
     * @see
     * com.puresol.coding.analysis.SLOCMetric#getProductiveLineStatistics()
     */

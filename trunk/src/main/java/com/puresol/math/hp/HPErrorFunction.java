...
public class HPErrorFunction {
	...
	private static BigDecimal calculate(BigDecimal x) {
		return BigDecimal.valueOf(2.0)
				.divide(HPMath.sqrt(BigDecimal.valueOf(Math.PI)))
				.multiply(sum(x));
	}
	...
	private static BigDecimal sum(BigDecimal x) {
		...
		for (int n = 0; n <= 100; n++) {
			result.add(x.divide(
					BigDecimal.valueOf(2.0).multiply(BigDecimal.valueOf(n))
							.add(BigDecimal.valueOf(1.0))).multiply(
					product(x, n)));
		}
		return result;
	}
	...
	public static BigDecimal erfc(BigDecimal y) {
		try {
			...
			if ((y.doubleValue() < 1.0) && (y.doubleValue() > -1.0)) {
				HPNewtonsMethod newton = new HPNewtonsMethod(
						new ErrorFunction(), "erf");
				return newton.find(y, BigDecimal.valueOf(-5.0),
						BigDecimal.valueOf(5.0), BigDecimal.valueOf(1e-9));
			}

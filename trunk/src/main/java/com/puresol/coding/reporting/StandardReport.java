...
public class StandardReport {
	...
	private void createLogoFile() {
		...
		if (logoFile != null) {
			File logo = new File(projectDirectory, logoFile.getPath());
			if (!logo.exists()) {
	...
	private void createCSSFile() {
		if (cssFile != null) {
			File css = new File(projectDirectory, cssFile.getPath());
			if (!css.exists()) {
				JarFileUtilities.extractResource(
	...
	private void createFavIconFile() {
		if (favIconFile != null) {
			File favIcon = new File(projectDirectory, favIconFile.getPath());
			if (!favIcon.exists()) {
				JarFileUtilities.extractResource(

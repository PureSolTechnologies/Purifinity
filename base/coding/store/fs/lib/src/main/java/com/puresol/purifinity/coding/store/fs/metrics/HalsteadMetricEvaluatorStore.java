...
public class HalsteadMetricEvaluatorStore extends AbstractEvaluatorStore {
	...
	@Override
	public HalsteadMetricFileResults readFileResults(HashId hashId) {
		if (hasFileResults(hashId)) {
			File file = getFileResultsFile(hashId);
			return restore(file, HalsteadMetricFileResults.class);
		} else {
			return null;
		}
	}
	...
	@Override
	public HalsteadMetricDirectoryResults readDirectoryResults(HashId hashId) {
		if (hasDirectoryResults(hashId)) {
			File file = getDirectoryResultsFile(hashId);
			return restore(file, HalsteadMetricDirectoryResults.class);
		} else {
			return null;
		}
	}
	...
	@Override
	public HalsteadMetricDirectoryResults readProjectResults(

...
public class VerticalData {
	...
	for (int column = 0; column < values.length; column++) {
	    if (columnTypes.get(column).getClassObject().isAssignableFrom(values[column].getClass())) {
		row.add(values[column]);
	    } else {
		try {
		    Method valueOf = columnTypes.get(column).getClassObject().getMethod("valueOf",
			    values[column].getClass());
		    row.add(valueOf.invoke(columnTypes.get(column), values[column]));
		    continue;
		...
		throw new IllegalArgumentException(
			"Class '" + values.getClass().getName() + "' is not castable to column type '"
				+ columnTypes.get(column).getClassName() + "' of column " + (column + 1) + "!");
	    }
	}
	rowData.add(row);
    }
	...
	try {
		...
		addColumn(name, ValueType.fromClassName(metaData.getColumnClassName(column + 1)));
	    }
     ...
     * 
     * @param columnName
     *            is the name of the column for which the id is to be returned.
     * @return An int is returned with the id of the column.
     */

...
public class EntropyMetricEvaluator extends AbstractMetricEvaluator {
	...
	super(EntropyMetric.ID, EntropyMetric.NAME, EntropyMetric.PLUGIN_VERSION, EntropyMetric.DESCRIPTION);
    }
	...
	EntropyFileResults entropyResults = new EntropyFileResults(hashId, sourceCodeLocation, new Date());

	List<HalsteadMetricResult> halsteadMetricResults = halsteadMetricEvaluatorDAO.readFileResults(hashId);
	...
	for (CodeRange codeRange : analysis.getAnalyzableCodeRanges()) {
		...
		if (halsteadMetricResult.getCodeRangeType().equals(codeRange.getType())
			&& halsteadMetricResult.getCodeRangeName().equals(codeRange.getCanonicalName())) {
		    EntropyMetricResult entropyResult = calculateEntropyResult(
			    halsteadMetricResult.getHalsteadResult());
		    entropyResults.add(new EntropyResult(sourceCodeLocation, codeRange.getType(),
			    codeRange.getCanonicalName(), entropyResult));
		}
	    }
	}
	return entropyResults;
	...
	EntropyMetricResult result = new EntropyMetricResult(halstead.getVocabularySize(), halstead.getProgramLength(),
		entropy, maxEntropy, normEntropy, entropyRedundancy, redundancy, normalizedRedundancy);
	return result;
    }
	...
	EntropyMetricResult entropyResult = calculateEntropyResult(halsteadDirectoryResults.getHalsteadResult());
	EntropyDirectoryResults directoryResults = new EntropyDirectoryResults(directory.getHashId(), new Date(),
		new UnspecifiedSourceCodeLocation(), CodeRangeType.DIRECTORY, directory.getName());

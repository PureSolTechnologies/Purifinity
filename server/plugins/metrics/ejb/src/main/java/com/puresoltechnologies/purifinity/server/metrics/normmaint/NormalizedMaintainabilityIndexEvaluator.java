...
public class NormalizedMaintainabilityIndexEvaluator extends AbstractMetricEvaluator {

    public static final String ID = NormalizedMaintainabilityIndexEvaluator.class.getName();
	...
	NormalizedMaintainabilityIndexFileResults results = new NormalizedMaintainabilityIndexFileResults(
		NormalizedMaintainabilityIndexEvaluator.ID, NormalizedMaintainabilityIndexEvaluator.PLUGIN_VERSION,
	...
	if (maintainabilityFileResults == null) {
	    logger.warn("No Maintainability Index result available for '" + hashId + "', yet.");
	    return results;
	}

	for (CodeRange codeRange : analysis.getAnalyzableCodeRanges()) {
	    MaintainabilityIndexFileResult maintainabilityIndexFileResult = findFileResult(maintainabilityFileResults,
		    codeRange);
	    ...
	    NormalizedMaintainabilityIndexResult result = new NormalizedMaintainabilityIndexResult(miWoc, miCw);

	    results.add(new NormalizedMaintainabilityIndexFileResult(sourceCodeLocation, codeRange.getType(),
		    codeRange.getCanonicalName(), result,
		    NormalizedMaintainabilityQuality.get(codeRange.getType(), result)));
	}
	return results;
    }

    private MaintainabilityIndexFileResult findFileResult(
	    List<MaintainabilityIndexFileResult> maintainabilityFileResults, CodeRange codeRange) {

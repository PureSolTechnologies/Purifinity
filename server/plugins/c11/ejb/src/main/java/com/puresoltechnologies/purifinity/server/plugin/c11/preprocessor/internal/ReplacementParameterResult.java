...
public class ReplacementParameterResult {
     ...
     * 
     * @param tokenStream
     *            is the {@link TokenStream} to be used to parse for
     *            replacement.
     * @param tokenId
     *            is the id of the token.
     ...
     *             is thrown in case the pre-processor has issues.
     */
    public static ReplacementParameterResult extractParameterReplacements(TokenStream tokenStream, int tokenId)
	    throws PreprocessorException {
	...
	for (int i = tokenId + 1; i < tokenStream.size(); i++) {
		...
		if (parameters.hasParameters() != null) {
		    ...
		    throw new PreprocessorException(
			    "An opening parenthesis was expected after macro name '" + startToken.getName()
				    + "' in line " + startToken.getMetaData().getLine() + ", but was not found!");

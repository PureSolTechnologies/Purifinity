...
public class C11PreprocessorExpressionEvaluator {
	...
	public void evaluate() throws UniversalSyntaxTreeEvaluationException,
			TreeException {
	...
	private Value evaluate(ParseTreeNode expression)
		...
		if ("expression".equals(nodeName)) {
			...
			throw new UniversalSyntaxTreeEvaluationException(
					"Unknown expression type '" + nodeName + "' ("
							+ expression.toString() + ")");
		}
	}
	...
	private Value generateValue(String identifierName)
		...
		if ((macro.getParameters().size() > 0)
			...
			throw new UniversalSyntaxTreeEvaluationException("Macro '"
					+ identifierName
					+ "' is a function like macro. This is not supported.");
		}
		if (macro.getReplacement().size() > 1) {
			throw new UniversalSyntaxTreeEvaluationException("Macro '"
					+ identifierName + "' has the complex replacement '"

...
public class AnalysisProcessStateTrackerImpl implements
	...
	AnalysisProcessStatusInformation state = readProcessState(projectId);
	if (state != null) {
	    throw new IllegalStateException(
		    "Process(es) was/were found for project UUID '" + projectId
			    + "'.");
	}
	PreparedStatement preparedStatement = preparedStatements
		.getPreparedStatement(
			session,
			"INSERT INTO analysis_process (started, last_progress, project_id, state) VALUES (?, ?, ?, ?);");
	...
	while (!resultSet.isExhausted()) {
	    ...
	    String projectId = row.getString(0);
	    long runId = row.getLong(1);
	...
	AnalysisProcessStatusInformation statusInformation = readProcessState(projectId);
	...
	setProcessState(projectId, runId, model.getState());
	return true;
    }

    private void setProcessState(String projectId, long runId,
	...
	PreparedStatement preparedStatement = preparedStatements
		.getPreparedStatement(session,
			"DELETE FROM analysis_process WHERE project_id = ?;");
	BoundStatement boundStatement = preparedStatement.bind(projectId);
	session.execute(boundStatement);
    }
}

...
import com.puresoltechnologies.purifinity.evaluation.domain.style.StyleIssueParameter;
import com.puresoltechnologies.purifinity.server.common.utils.PropertiesUtils;
import com.puresoltechnologies.purifinity.server.core.api.evaluation.style.EvaluatorStyleIssuesStoreService;
import com.puresoltechnologies.purifinity.server.core.api.evaluation.style.EvaluatorStyleIssuesStoreServiceRemote;
...
@Stateless
public class EvaluatorStyleIssuesStoreServiceBean
	implements EvaluatorStyleIssuesStoreService, EvaluatorStyleIssuesStoreServiceRemote {
	...
	try (PreparedStatement preparedStatement = connection.prepareStatement("SELECT hashid FROM "
		+ HBaseElementNames.EVALUATION_STYLE_ISSUES_TABLE
	...
	try (PreparedStatement preparedStatement = connection.prepareStatement("SELECT hashid FROM "
		+ HBaseElementNames.EVALUATION_STYLE_ISSUES_TABLE + " WHERE hashid=? AND evaluator_id=?")) {
	    preparedStatement.setString(1, hashId.toString());
    ...
    @Override
    public void storeFileResults(AnalysisRun analysisRun, CodeAnalysis codeAnalysis, FileStyleIssues results)
	    throws EvaluationStoreException {
	try (PreparedStatement preparedStatement = connection.prepareStatement("UPSERT INTO "
		+ HBaseElementNames.EVALUATION_STYLE_ISSUES_TABLE + " (time, " + "hashid, " + "source_code_location, "
		...
		for (StyleIssueParameter parameter : results.getParameters()) {
		    ...
		    List<StyleIssue> issues = codeRangeIssues.getIssues(parameter);
		    if (issues == null) {
    ...
    @Override
    public void storeDirectoryResults(AnalysisRun analysisRun, AnalysisFileTree directory, DirectoryStyleIssues results)
	    throws EvaluationStoreException {
	throwUnsupportedException();
    }

    @Override
    public void storeProjectResults(AnalysisRun analysisRun, AnalysisFileTree directory, ProjectStyleIssues results)
	    throws EvaluationStoreException {
	throwUnsupportedException();
    }

    @Override
    public FileStyleIssues readFileResults(HashId hashId, String evaluatorId) throws EvaluationStoreException {
    ...
    @Override
    public DirectoryStyleIssues readDirectoryResults(HashId hashId, String evaluatorId)
	    throws EvaluationStoreException {
	throwUnsupportedException();
	return null;
    }
    ...
    @Override
    public ProjectStyleIssues readProjectResults(String projectId, long runId, String evaluatorId)
	    throws EvaluationStoreException {
	throwUnsupportedException();
	return null;
    }
    ...
    @Override
    public void storeFileResultsInBigTable(AnalysisRun analysisRun, CodeAnalysis codeAnalysis, FileStyleIssues results)
	    throws EvaluationStoreException {
    ...
    public void storeDirectoryResultsInBigTable(AnalysisRun analysisRun, AnalysisFileTree directory,
	    DirectoryStyleIssues results) throws EvaluationStoreException {
	throwUnsupportedException();
    }

    @Override
    public void storeProjectResultsInBigTable(AnalysisRun analysisRun, AnalysisFileTree directory,
	    ProjectStyleIssues metrics) throws EvaluationStoreException {
	throwUnsupportedException();
    }

    @Override
    public RunStyleIssues readRunResults(String projectId, long runId, String evaluatorId)
	    throws EvaluationStoreException {
	throwUnsupportedException();
	return null;
    }

...
import com.puresoltechnologies.purifinity.evaluation.domain.architecture.ArchitectureIssue;
...
import com.puresoltechnologies.purifinity.server.core.api.evaluation.architecture.EvaluatorArchitectureIssuesStoreService;
import com.puresoltechnologies.purifinity.server.core.api.evaluation.architecture.EvaluatorArchitectureIssuesStoreServiceRemote;
...
@Stateless
public class EvaluatorArchitectureIssuesStoreServiceBean
	implements EvaluatorArchitectureIssuesStoreService, EvaluatorArchitectureIssuesStoreServiceRemote {
	...
	try (PreparedStatement preparedStatement = connection.prepareStatement("SELECT hashid FROM "
		+ HBaseElementNames.EVALUATION_ARCHITECTURE_ISSUES_TABLE
	...
	try (PreparedStatement preparedStatement = connection.prepareStatement("SELECT hashid FROM "
		+ HBaseElementNames.EVALUATION_ARCHITECTURE_ISSUES_TABLE + " WHERE hashid=? AND evaluator_id=?")) {
	    preparedStatement.setString(1, hashId.toString());
    ...
    @Override
    public void storeFileResults(AnalysisRun analysisRun, CodeAnalysis codeAnalysis, FileArchitectureIssues results)
	    throws EvaluationStoreException {
	try (PreparedStatement preparedStatement = connection
		.prepareStatement("UPSERT INTO " + HBaseElementNames.EVALUATION_ARCHITECTURE_ISSUES_TABLE + " (time, "
			...
			+ "start_line, " + "start_column, " + "line_count, " + "length" + ") VALUES "
			+ "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
		...
		for (ArchitectureIssueParameter parameter : results.getParameters()) {
		    ...
		    List<ArchitectureIssue> issues = codeRangeIssues.getIssues(parameter);
		    if (issues == null) {
    ...
    public void storeDirectoryResults(AnalysisRun analysisRun, AnalysisFileTree directory,
	    DirectoryArchitectureIssues results) throws EvaluationStoreException {
	throwUnsupportedException();
    }

    @Override
    public void storeProjectResults(AnalysisRun analysisRun, AnalysisFileTree directory,
	    ProjectArchitectureIssues results) throws EvaluationStoreException {
	throwUnsupportedException();
    }

    @Override
    public FileArchitectureIssues readFileResults(HashId hashId, String evaluatorId) throws EvaluationStoreException {
	try (PreparedStatement preparedStatement = connection.prepareStatement("SELECT " + "time, "
		...
		+ "source_code_location " + "FROM " + HBaseElementNames.EVALUATION_ARCHITECTURE_ISSUES_TABLE
    ...
    @Override
    public DirectoryArchitectureIssues readDirectoryResults(HashId hashId, String evaluatorId)
	    throws EvaluationStoreException {
	throwUnsupportedException();
	return null;
    }
    ...
    @Override
    public ProjectArchitectureIssues readProjectResults(String projectId, long runId, String evaluatorId)
	    throws EvaluationStoreException {
	throwUnsupportedException();
	return null;
    }
    ...
    public void storeFileResultsInBigTable(AnalysisRun analysisRun, CodeAnalysis codeAnalysis,
	    FileArchitectureIssues results) throws EvaluationStoreException {
    ...
    public void storeProjectResultsInBigTable(AnalysisRun analysisRun, AnalysisFileTree directory,
	    ProjectArchitectureIssues metrics) throws EvaluationStoreException {
	throwUnsupportedException();
    }

    @Override
    public RunArchitectureIssues readRunResults(String projectId, long runId, String evaluatorId)
	    throws EvaluationStoreException {
	throwUnsupportedException();
	return null;
    }

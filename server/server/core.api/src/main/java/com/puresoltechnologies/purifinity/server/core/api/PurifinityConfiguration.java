...
public interface PurifinityConfiguration {
    ...
    public static final ConfigurationParameter<Boolean> ANONYMOUS_CAN_READ = new ConfigurationParameter<>(
	    "Anonymous can read", "", LevelOfMeasurement.NOMINAL,
    ...
    public static final ConfigurationParameter<Integer> ALERT_MESSAGE_TIMEOUT = new ConfigurationParameter<>(
	    ...
	    "Specifies the timeout for messages shown by Alerter.", Integer.class, "webui.messages.timeout", "/WebUI",
	    3);

    public static final ConfigurationParameter<Integer> USER_INACTIVITY_TIMEOUT = new ConfigurationParameter<>(
	    "User Inactivity Timeout", "min", LevelOfMeasurement.RATIO,
	    "Specifies the time in minutes after which an inactive user is automatically logged off.", Integer.class,
	    "user.login.timeout.inactive", "/Security", 60);

    public static final ConfigurationParameter<Integer> USER_SESSION_TIMEOUT = new ConfigurationParameter<>(
	    "User Session Timeout", "min", LevelOfMeasurement.RATIO,
    ...
    public static final ConfigurationParameter<Long> MAX_FILE_SIZE = new ConfigurationParameter<>("Maximum File Size",
	    "byte", LevelOfMeasurement.RATIO, "Specifies the maximum file size which shall be stored into databse..",

...
public class EncryptionUtilities {
     ...
     * 
     * @param bytes
     *            is an array of bytes which need to be converted to an
     *            hexadezimal {@link String}.
     * @return A {@link String} with the hexadecimal representation of the byte
     *         array.
     */
    public static String convertBytesToHexString(byte[] bytes) {
	StringBuilder buffer = new StringBuilder();
	for (int i = 0; i < bytes.length; i++) {
	    buffer.append(convertByteToHexString(bytes[i]));
	}
     ...
     * 
     * @param b
     *            is the byte which needs to be converted to an hexadecimal
     *            {@link String}.
     * @return A {@link String} with the hexadecimal representation of the byte.
     */
     ...
     * 
     * @param halfByte
     *            is the half byte which needs to be converted to an hexadecimal
     *            {@link String}.
     * @return A {@link String} with the hexadecimal representation of the half
     *         byte.
     */
    static char convertHalfByteToHexCharacter(byte halfByte) {
	if ((halfByte < 0) || (halfByte > 15))
	    throw new IllegalArgumentException("Half bytes need to be >=0 and <=15!");
	if (halfByte < 10) {

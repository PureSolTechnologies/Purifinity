...
public class PasswordStoreClientIT extends AbstractPasswordStoreClientTest {
	...
	String activationKey = passwordStoreClient.createPassword(
	...
	String activationKey = passwordStoreClient.createPassword(
		EMAIL_ADDRESS, new Password("12dqwec1241`S@#R~"));
	assertNotNull(activationKey);
	assertFalse(activationKey.isEmpty());
	assertEquals(EMAIL_ADDRESS, passwordStoreClient.activatePassword(
		EMAIL_ADDRESS, activationKey));
    }
	...
	String activationInformation = passwordStoreClient.createPassword(
		EMAIL_ADDRESS, VALID_PASSWORD);
	passwordStoreClient.activatePassword(EMAIL_ADDRESS,
	...
	assertFalse(passwordStoreClient.authenticate(new EmailAddress(
		EMAIL_ADDRESS + "Wrong"), VALID_PASSWORD));
    }
	...
	assertTrue(passwordStoreClient.authenticate(EMAIL_ADDRESS,
		VALID_PASSWORD));
	assertTrue(passwordStoreClient.changePassword(EMAIL_ADDRESS,
		VALID_PASSWORD, new Password(VALID_PASSWORD.getPassword()
			+ "New!")));
	assertTrue(passwordStoreClient.authenticate(EMAIL_ADDRESS,
	...
	assertTrue(passwordStoreClient.authenticate(EMAIL_ADDRESS,
		VALID_PASSWORD));
	assertFalse(passwordStoreClient.changePassword(new EmailAddress(
		EMAIL_ADDRESS + "Wrong"), VALID_PASSWORD, new Password(
		VALID_PASSWORD + "New!")));
    }
	...
	assertTrue(passwordStoreClient.authenticate(EMAIL_ADDRESS,
		VALID_PASSWORD));
	assertFalse(passwordStoreClient.changePassword(EMAIL_ADDRESS,
		new Password(VALID_PASSWORD + "Wrong!"), new Password(
			VALID_PASSWORD + "New!")));
    }
	...
	assertTrue(passwordStoreClient.authenticate(EMAIL_ADDRESS,
		VALID_PASSWORD));
	passwordStoreClient.changePassword(EMAIL_ADDRESS, VALID_PASSWORD,
		TOO_WEAK_PASSWORD);
    }

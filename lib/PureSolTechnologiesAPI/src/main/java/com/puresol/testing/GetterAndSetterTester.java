...
import com.puresol.gui.data.RandomNumbers;
import com.puresol.gui.data.TypeWrapper;
import com.puresol.utils.IntrospectionUtilities;

public class GetterAndSetterTester {

    private static final Logger logger = LoggerFactory
	    .getLogger(GetterAndSetterTester.class);
	...
	if (name.toLowerCase().getBytes()[0] != name.getBytes()[0]) {
	    logger.error("In class " + clazz.getName() + " the field " + name
	...
	try {
		...
		logger.error("In class " + clazz.getName() + " the field "
			+ field.getName() + " has an invalid getter method!");
		return false;
	    }
	    return true;
	...
	try {
		...
		logger.error("In class " + clazz.getName() + " the field "
			+ field.getName() + " has an invalid setter method!");
		return false;
	    }
	    return true;
	} catch (SecurityException e) {
	...
	try {
	    ...
	    logger.warn("Cannot check correct internal assignment of getters and setters due to missing default constructor!");
	    return true;
	...
	try {
		...
		if (field.getAnnotation(NoGetterAndSetterValueTest.class) != null) {
		    /*
		     * do not proceed, field is marked as no getter and setter
		     * safe!
		     */
		    continue;
		}

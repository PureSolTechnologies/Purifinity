...
/**
 * This login module is designed for LDAP authentication against a LDAP
 * server. It's an abstract class for inheritance to a meaningful class
 * with valid connection information and content handling.
 * 
...
abstract public class AbstractLdapLoginModule implements LoginModule {
    ...
    private static final Logger logger =
	    Logger.getLogger(AbstractLdapLoginModule.class);
	...
	try {
	    ...
	    Hashtable<String, String> env =
		    createLDAPEnvironmentProperties(callbacks);
	...
	callbacks.put(PasswordCallback.class, new PasswordCallback(
		"password", false));
	return callbacks;
    }

    private Callback[] callbacks2Array(
	    Hashtable<Class<?>, Callback> callbacks) {
    ...
    /**
     * This method sets all properties needed to connect to the LDAP server
     * into a hashtable,
     * 
     * @param callbacks
     *            are the current callbacks holding all information about
     *            the login.
    ...
    abstract protected void updateSubjectInformation(
	    SubjectInformation subjectInformation, DirContext ctx,
	    String userID) throws LoginException;

    /**
     * This method checks the current principals for a SubjectInformation
     * object and returns it if present. If there is none found, a new
     * instance of SubjectInformation is returned.
     * 
	...
	try {
		...
		Attribute attribute =
			(Attribute) attributesEnumeration.next();
